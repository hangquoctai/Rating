  ...
  $(document).ready(function () {
    ...
    function getPage(total, object) {
      ...
      for (var i = 1; i <= total; i++) {
        htmlPae += `<li><a href="javascript:void(0);" data-href=${mainUrl}/admin/api/review/{{product.id}}?orgid=${orgidShop}&page=${i}&size=${objConfig.numberPaginate}>${i}</a></li>`;
      }
    ...
    function dataRender(res) {
      ...
      $.each(dataNew, function (i, v) {
        if(v.status > 0){
          ...
          let dateString = new Date(v.createdAt).toLocaleString('vi-vn',options);
          let html = ''
          ...
          objConfig.isShare ? html += `<a class="shareReview">Chia sẻ Facebook</a>` : '';
          /*objConfig.isGoodAnswer ? html += `<a class="goodReview">Nhận xét này hữu ích với bạn?</a></div></div></div>` : '';*/
    ...
    function dataCheckStar(res) {
      ...
      if(dataNew != null){ 
        ...
        if (dataNew.ratingAverage % 1 == 0){
          ...
          $("head").append(`<style>.starPoint i.half:after{
...
overflow: hidden; 
        ...
        $('.filterRating[data-filter="starCount=5"]').html(`5 sao (${dataNew.oneStar})`).attr('data-count',`${dataNew.oneStar}`);
        $('.filterRating[data-filter="starCount=4"]').html(`4 sao (${dataNew.twoStar})`).attr('data-count',`${dataNew.twoStar}`);
        $('.filterRating[data-filter="starCount=3"]').html(`3 sao (${dataNew.threeStar})`).attr('data-count',`${dataNew.threeStar}`);
        $('.filterRating[data-filter="starCount=2"]').html(`2 sao (${dataNew.fourStar})`).attr('data-count',`${dataNew.fourStar}`);
        $('.filterRating[data-filter="starCount=1"]').html(`1 sao (${dataNew.fiveStar})`).attr('data-count',`${dataNew.fiveStar}`); 
    ...
    $(document).on('click', '.selectReview i', function (e) {
      let flagTarget = e.target.getAttribute('number');
      $(`.selectReview i:nth-child(-n + ${flagTarget})`).addClass('activeOk').css('color',`${objConfig.colorImgReview}`)
    })
    ...
    $('#uploadReview').submit(function (e) {
      ...
      console.log(typeof parseProductId)
      ...
      formData.append('email', $(this).find('input#emailReview').val())
      formData.append('phone', $(this).find('input#phoneReview').val())
