...
var orgidShop = null;
	...
	$.get(`${shopUrlF1Review}/meta.json`).done(function (data) {
	...
	$(document).ready(function () {
		...
		function getPage(total, object) {
			...
			for (var i = 1; i <= total; i++) {
				htmlPae += `<li><a href="javascript:void(0);" data-href=${mainUrl}/admin/api/review/list/${productIdF1Review}?orgid=${orgidShop}&page=${i}&size=${objConfig.numberPaginate}>${i}</a></li>`;
			}
...
overflow: hidden;	
		...
		function myApp() {
			if(objConfig != null){
				getData(`${mainUrl}/admin/api/review/list/${productIdF1Review}?orgid=${orgidShop}&page=1&size=${objConfig.numberPaginate}`).then(function (res) {   
					dataRender(res);
				})
				getData(`${mainUrl}/admin/api/product/list/${productIdF1Review}?orgid=${orgidShop}`).then(function (res) {
					dataCheckStar(res);
				});
		...
		$('#uploadReview').submit(function (e) {
			e.preventDefault();
			$('#errorList').html('');
			let parseProductId = parseInt(productIdF1Review)
			let formData = new FormData(this);
			formData.append('orgid', orgidShop)
			formData.append('productId', parseProductId)
			formData.append('productName', productTitleF1Review)
			formData.append('productImage', productImageF1Review)
			formData.append('productUrl', 'shopUrlF1Review + productUrlF1Review')
